 ...
 * <ul>
 *     <li>{@link PixelArt} for token-grid rendering when you already have a
 *     finished {@code String[][]} source grid, including grids loaded from a
 *     text file</li>
 *     <li>{@link PixelColorArt} for direct rendering of exact
 *     {@link Colors.Color} cell values when you do not want palette tokens at
 *     all</li>
 * </ul>
 *
 * <p>Most manual art code starts with {@link PixelGridBuilder}, then ends here
 * through {@link #render(PixelGridBuilder)}. Both render paths share the same
 * output-size resolution, optional border handling, in-memory rendering model,
 * and single final PNG write.</p>
 */
public final class PixelGenerator {
     ...
     * {@link PixelArt}-based validation and PNG write path. Use this when you
     * built the image through draw commands such as {@code setCell},
     * {@code drawLine}, or {@code fillRect}.</p>
     *
     ...
     *
     * <p>Use this when you already have the final token grid in memory, such
     * as from {@link PixelImageQuantizer#loadTextGrid(String)} or from your own
     * array-building code, and you do not need the mutable builder API.</p>
     *
     * <p>Render behavior:</p>
     * <ul>
     ...
     * {@link Colors.Color} cell values instead of palette tokens. Use this only
     * when you need exact RGB control rather than the shared token palette.</p>
     *
