...
public final class QueueManager {
    ...
    public static final class Config {
        ...
        public static volatile long CACHE_TTL_MILLIS = 2 * 60_000; // 2 minutes

        /**  Long idle → compact/materialize (write base + truncate journal) */
        public static volatile int MATERIALIZE_INACTIVE_SECS = 600; // ~10 minutes default
    ...
    /**
     * Refreshes queue runtime tunables from ConfigManager.
     *
     * This keeps QueueManager's existing public config surface intact while
     * centralizing its defaults in the runtime config JSON. The values are read
     * into the existing static fields so current behavior is preserved unless the
     * config changes.
     */
    public static synchronized void applyRuntimeConfig() {
        Config.CACHE_TTL_MILLIS = Math.max(0L, ConfigManager.getLong("queue_manager.cache_ttl_millis", 120_000L));
        Config.MATERIALIZE_INACTIVE_SECS = Math.max(1, ConfigManager.getInt("queue_manager.materialize_inactive_secs", 600));
        Config.CACHE_MAX_FILES = Math.max(0, ConfigManager.getInt("queue_manager.cache_max_files", 0));
        Config.CACHE_MEMORY_CAP_BYTES = Math.max(0L, ConfigManager.getLong("queue_manager.cache_memory_cap_bytes", 500L * 1024 * 1024));
        Config.BATCH_JOB_FLUSH_THRESHOLD = Math.max(1, ConfigManager.getInt("queue_manager.batch_job_flush_threshold", 100));
        Config.WORKER_IDLE_POLL_MS = Math.max(1L, ConfigManager.getLong("queue_manager.worker_idle_poll_ms", 5_000L));
        Config.WORKER_REGULAR_REQUEST_BURST = Math.max(1, ConfigManager.getInt("queue_manager.worker_regular_request_burst", 4));
        Config.MAX_IMMEDIATE_RETRIES = Math.max(0, ConfigManager.getInt("queue_manager.max_immediate_retries", 1));
        Config.MAX_TOTAL_RETRIES = Math.max(1, ConfigManager.getInt("queue_manager.max_total_retries", 5));
        Config.BACKOFF_BASE_MS = Math.max(0L, ConfigManager.getLong("queue_manager.backoff_base_ms", 150L));
        Config.BACKOFF_MAX_MS = Math.max(Config.BACKOFF_BASE_MS, ConfigManager.getLong("queue_manager.backoff_max_ms", 2_000L));
        Config.BACKOFF_JITTER_MS = Math.max(0L, ConfigManager.getLong("queue_manager.backoff_jitter_ms", 50L));
        Config.JOURNAL_MAX_BYTES = Math.max(0L, ConfigManager.getLong("queue_manager.journal_max_bytes", 16L * 1024 * 1024));
        Config.JOURNAL_MAX_RECORDS = Math.max(0L, ConfigManager.getLong("queue_manager.journal_max_records", 0L));
        Config.SHUTDOWN_AWAIT_MS = Math.max(0L, ConfigManager.getLong("queue_manager.shutdown_await_ms", 15_000L));
    }
    ...
    public static final class RawIO {
        ...
        static void appendPatch(String path, Map<String, Object> diff) throws QueueException {
            ...
            try {
                ...
                boolean overBytes = (bytes > QueueManager.Config.JOURNAL_MAX_BYTES);
