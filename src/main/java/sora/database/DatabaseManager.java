package sora.database;

import org.json.JSONArray;
import org.json.JSONObject;
import sora.exceptions.BatchException;
...
/**
 * Basic file-based database manager for Sora.
...
public class DatabaseManager {
    ...
    private static final String ROOT_DIR = BootstrapConfig.DATABASE_ROOT.toString();
...
// =====================
    ...
    /**
     * Reads the full JSON object for a file and returns a defensive copy.
     *
     * Behavior:
     * - Validates that the target file path is non-null and non-blank.
     * - Performs an integrity check before reading so callers receive a validated
     *   object view that conforms to DBM's object-root policy.
     * - Reads through QueueManager so the returned object reflects the current
     *   authoritative cache state, including queued / journal-backed changes.
     * - Returns a new JSONObject copy, never the live cached object reference.
     *
     * Policy:
     * - This method is object-root only.
     * - Integrity validation does not auto-repair here; failures are surfaced to
     *   the caller as DatabaseException.
     *
     * Logging:
     * - DEBUG when the read starts.
     * - INFO when the read completes successfully.
     * - ERROR with DBM error codes when validation, queue access, or object-copy
     *   creation fails.
     *
     * Examples:
     *   JSONObject bot = DatabaseManager.readJSONObject("database/global/bot.json");
     *   JSONObject config = DatabaseManager.readJSONObject("database/global/config.json");
     *
     * @param filePath the target JSON file path.
     * @return defensive copy of the full JSON object for the file.
     * @throws DatabaseException if the path is invalid, integrity validation fails,
     *                           queue access fails, or the cached object cannot be copied.
     */
    public static JSONObject readJSONObject(String filePath) throws DatabaseException {
        if (filePath == null || filePath.isBlank()) {
            Logger.log(Logger.TAG.ERROR, "[01181] readJSONObject: file path is null or empty.");
            throw new DatabaseException("readJSONObject: file path is null or empty.");
        }

        Logger.log(Logger.TAG.DEBUG, "Reading full JSON object from file: " + filePath);

        try {
            ensureJSONIntegrity(filePath, true, false);

            JSONObject result = QueueManager.readValue(
                    filePath,
                    null,
                    root -> {
                        if (root == null) {
                            throw new DatabaseException("readJSONObject: cache returned null JSON object.");
                        }
            ...
            if (result == null) {
                Logger.log(Logger.TAG.ERROR, "[01182] readJSONObject: queue returned null object for " + filePath);
                throw new DatabaseException("readJSONObject: queue returned null JSON object.");
            }

            Logger.log(Logger.TAG.INFO, "Successfully read full JSON object from file: " + filePath);
            return result;

        } catch (QueueException e) {
            Logger.log(Logger.TAG.ERROR, "[01183] Queue read error in readJSONObject for " + filePath + ": " + e.getMessage());
            throw new DatabaseException("readJSONObject: queue read error for file: " + filePath, e);
        } catch (DatabaseException e) {
            Logger.log(Logger.TAG.ERROR, "[01184] readJSONObject failed for " + filePath + ": " + e.getMessage());
            throw e;
        } catch (Exception e) {
            Logger.log(Logger.TAG.ERROR, "[01185] Unexpected error in readJSONObject for " + filePath + ": " + e.getMessage());
            throw new DatabaseException("readJSONObject: unexpected error while copying cached JSON object.", e);
        }
    }
...
// =====================
    ...
    public static void moveToCorrupt(String path) throws Exception {
        ...
        java.nio.file.Path corruptRoot = java.nio.file.Path.of(BootstrapConfig.CORRUPTPATH).toAbsolutePath().normalize();
