...
public final class CrashHandler {
    private static final int DEFAULT_JOURNAL_CLEAR_TIMEOUT_MS = 10_000;
    private static final int DEFAULT_JOURNAL_POLL_INTERVAL_MS = 50;
    ...
    public static Result checkAndRecover() throws DatabaseException, QueueException {
        ...
        final int journalClearTimeoutMs = journalClearTimeoutMs();
        final int journalPollIntervalMs = journalPollIntervalMs();
        ...
        for (Path base : bases) {
            ...
            if (!waitForJournalClear(journal, journalClearTimeoutMs, journalPollIntervalMs)) {
    ...
    /** Returns the crash-recovery journal clear timeout with coded fallback. */
    private static int journalClearTimeoutMs() {
        return Math.max(1, ConfigManager.getInt("crash_handler.journal_clear_timeout_ms", DEFAULT_JOURNAL_CLEAR_TIMEOUT_MS));
    }

    /** Returns the crash-recovery journal poll interval with coded fallback. */
    private static int journalPollIntervalMs() {
        return Math.max(1, ConfigManager.getInt("crash_handler.journal_poll_interval_ms", DEFAULT_JOURNAL_POLL_INTERVAL_MS));
    }
