...
final class ArtifactEditorWindow {
    ...
    private JScrollPane buildItemPanel() {
        JPanel itemPanel = new JPanel();
        itemPanel.setOpaque(false);
        itemPanel.setLayout(new BoxLayout(itemPanel, BoxLayout.Y_AXIS));
        for (DungeonItemDefinition definition : ArtifactItemLibrary.placeableDefinitions()) {
            JButton button = smallButton(definition.name());
            button.setAlignmentX(Component.LEFT_ALIGNMENT);
            button.setMaximumSize(new Dimension(Integer.MAX_VALUE, 24));
            button.setToolTipText(definition.kind().name().toLowerCase()
                    + " / " + definition.category().name().toLowerCase()
                    + " / " + definition.placement().name().toLowerCase());
            button.addActionListener(e -> {
                state.setSelectedItemId(definition.id());
