...
public final class ArtifactTemplateDraft {
    ...
    public ArtifactDirection preferredWallDirectionForItem(DungeonItemDefinition definition, ArtifactPoint cell) {
        if (definition == null || cell == null || !isWallPreferred(definition)) {
            return null;
        }
        return ArtifactEditorGeometry.flatWallDirectionForTouchedCell(cell, walls);
    }

    public boolean addDirectedLibraryItem(DungeonItemDefinition definition, ArtifactPoint cell, ArtifactDirection direction) {
        if (definition == null || cell == null || !definition.isMapBased()) {
            return false;
        }
        addItem(new ArtifactItem(definition.id(), cell, direction));
        return true;
    }

    public boolean addLibraryItem(DungeonItemDefinition definition, ArtifactPoint point, ArtifactPoint cell) {
        if (definition == null) {
            return false;
        }
        if (!definition.isMapBased()) {
            return false;
        }
        ArtifactPoint target = definition.isGridBased() ? point : cell;
        if (target == null) {
            return false;
        }
        if (definition.requiresWall()) {
            return addWallMountedItem(definition.id(), target);
        }
        if (isDirectional(definition)) {
            ArtifactDirection wallDirection = preferredWallDirectionForItem(definition, target);
            if (wallDirection != null) {
                return addDirectedLibraryItem(definition, target, wallDirection);
            }
            return false;
        }
        addItem(new ArtifactItem(definition.id(), target, ArtifactDirection.NORTH));
        return true;
    }

    public static boolean isDirectional(DungeonItemDefinition definition) {
        return definition != null &&
                (definition.requiresWall() || Boolean.TRUE.equals(definition.defaultProperties().get("directional")));
    }

    private static boolean isWallPreferred(DungeonItemDefinition definition) {
        return Boolean.TRUE.equals(definition.defaultProperties().get("prefers_wall"));
    }
