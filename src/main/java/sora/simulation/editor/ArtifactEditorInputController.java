...
final class ArtifactEditorInputController implements Runnable {
    ...
    private void handleLeftPress(int screenX, int screenY) {
        ...
        switch (tool) {
            ...
            case ITEM -> handleItemPlacement(point, cell);
            case ERASER -> {
                state.snapshotForUndo();
    ...
    private void handleItemPlacement(ArtifactPoint point, ArtifactPoint cell) {
        DungeonItemDefinition definition = ArtifactItemLibrary.require(state.getSelectedItemId());
        if (definition.requiresWall()) {
            placeItemNow(definition, point, cell);
            return;
        }
        ...
        if (pendingTool == ArtifactEditorTool.ITEM && pendingStart != null) {
            state.snapshotForUndo();
            boolean placed = state.getDraft().addDirectedLibraryItem(
                    definition,
                    pendingStart,
                    directionFromCellCenter(pendingStart, point)
            );
            if (!placed) {
                state.undo();
                Logger.log(Logger.TAG.DEBUG, "ArtifactEditorInputController: directed item placement rejected for id="
                        + definition.id() + ".");
            } else {
                state.clearSelection();
            }
        ...
        ArtifactDirection wallDirection = state.getDraft().preferredWallDirectionForItem(definition, cell);
        if (wallDirection != null) {
            state.snapshotForUndo();
            boolean placed = state.getDraft().addDirectedLibraryItem(definition, cell, wallDirection);
            if (!placed) {
                state.undo();
                Logger.log(Logger.TAG.DEBUG, "ArtifactEditorInputController: wall-directed item placement rejected for id="
                        + definition.id() + ".");
            } else {
                state.clearSelection();
            }
        ...
        pendingStart = cell;
        pendingTool = ArtifactEditorTool.ITEM;
        canvas.setPreview(cell, point);
    }

    private void placeItemNow(DungeonItemDefinition definition, ArtifactPoint point, ArtifactPoint cell) {
        state.snapshotForUndo();
        boolean placed = state.getDraft().addLibraryItem(definition, point, cell);
        if (!placed) {
            state.undo();
            Logger.log(Logger.TAG.DEBUG, "ArtifactEditorInputController: item placement rejected for id="
                    + definition.id() + ".");
        } else {
            state.clearSelection();
        }
    ...
    private ArtifactDirection directionFromCellCenter(ArtifactPoint cell, ArtifactPoint end) {
