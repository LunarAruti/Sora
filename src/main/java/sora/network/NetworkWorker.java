...
final class NetworkWorker implements Runnable {
    ...
    /** Coded fallback for max time (ms) we are willing to hold a task for an open circuit. */
    ...
    /**
     * Resolves the maximum window for keeping a task queued while its circuit
     * breaker remains open.
     *
     * Runtime behavior:
     * - Read from ConfigManager on demand so config edits can affect future
     *   retry scheduling without recreating the worker.
     * - Fall back to the coded default when config is missing or invalid.
     * - Clamp negative values to zero so hold behavior can be disabled safely.
     */
    private long maxCircuitHoldMs() {
        return Math.max(0L, ConfigManager.getLong("network_worker.max_circuit_hold_ms", MAX_CIRCUIT_HOLD_MS));
    }
    ...
    private void executeWithRetries(NetworkTask initialTask) {
        ...
        while (true) {
            ...
            if (!CircuitBreakerRegistry.allowRequest(circuitKey, now)) {
                ...
                if (remaining > 0 && remaining <= maxCircuitHoldMs()) {
