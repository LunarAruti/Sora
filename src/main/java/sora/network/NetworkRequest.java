...
public final class NetworkRequest {
    ...
    public synchronized NetworkRequest seal() {
        ...
        if (cachePath == null || cachePath.isBlank()) {
            ...
            this.cachePath = Path.of(
                    BootstrapConfig.NETWORKPATH,
                    service,
                    returnAlias + "-" + fp + ".json"
            ).toString();
    ...
    /** Applies config-backed request defaults to newly created request objects. */
    private void applyConfiguredDefaults() {
        this.type = parseEnum(
                NetworkRequest.Type.class,
                ConfigManager.getString("network_request_defaults.type", Type.GET.name()),
                Type.GET
        );
        this.authStrategy = parseEnum(
                NetworkRequest.AuthStrategy.class,
                ConfigManager.getString("network_request_defaults.auth_strategy", AuthStrategy.NONE.name()),
                AuthStrategy.NONE
        );
        this.timeout = Duration.ofMillis(Math.max(100L, ConfigManager.getLong("network_request_defaults.timeout_ms", 20_000L)));
        this.priority = parseEnum(
                NetworkRequest.Priority.class,
                ConfigManager.getString("network_request_defaults.priority", Priority.NORMAL.name()),
                Priority.NORMAL
        );
        this.failureMode = parseEnum(
                NetworkRequest.FailureMode.class,
                ConfigManager.getString("network_request_defaults.failure_mode", FailureMode.FAIL_FAST.name()),
                FailureMode.FAIL_FAST
        );
        this.responseType = parseEnum(
                NetworkRequest.ResponseType.class,
                ConfigManager.getString("network_request_defaults.response_type", ResponseType.JSON_OBJECT.name()),
                ResponseType.JSON_OBJECT
        );
        this.treatOtherStatusAsError = ConfigManager.getBoolean("network_request_defaults.treat_other_status_as_error", true);
        this.followRedirects = ConfigManager.getBoolean("network_request_defaults.follow_redirects", true);
        this.maxRedirects = Math.max(0, ConfigManager.getInt("network_request_defaults.max_redirects", 5));
        this.collectMetrics = ConfigManager.getBoolean("network_request_defaults.collect_metrics", true);
        this.retryPolicy = buildConfiguredRetryPolicy();
    }

    /** Builds the default retry policy for new requests from config with coded fallback. */
    private static RetryPolicy buildConfiguredRetryPolicy() {
        RetryMode mode = parseEnum(
                RetryMode.class,
                ConfigManager.getString("network_request_defaults.retry_policy.mode", RetryMode.IDEMPOTENT_ONLY.name()),
                RetryMode.IDEMPOTENT_ONLY
        );
        int maxAttempts = Math.max(1, ConfigManager.getInt("network_request_defaults.retry_policy.max_attempts", 3));
        long initialBackoffMs = Math.max(0L, ConfigManager.getLong("network_request_defaults.retry_policy.initial_backoff_ms", 250L));
        double backoffFactor = ConfigManager.getDouble("network_request_defaults.retry_policy.backoff_factor", 2.0d);
        Set<Integer> retryOnStatus = parseIntegerSet(
                ConfigManager.getValue("network_request_defaults.retry_policy.retry_on_status"),
                Set.of(429, 500, 502, 503, 504)
        );
        boolean retryOnTimeout = ConfigManager.getBoolean("network_request_defaults.retry_policy.retry_on_timeout", true);
        boolean retryOnNetworkError = ConfigManager.getBoolean("network_request_defaults.retry_policy.retry_on_network_error", true);
    ...
    /** Parses an enum name safely, falling back when config is missing or invalid. */
    ...
    /** Parses a config JSON array into an integer set with fallback. */
    private static Set<Integer> parseIntegerSet(Object raw, Set<Integer> fallback) {
        if (!(raw instanceof JSONArray arr) || arr.length() == 0) {
            return fallback;
        }
        ...
        for (int i = 0; i < arr.length(); i++) {
            ...
            if (value instanceof String stringValue) {
                try {
                    parsed.add(Integer.parseInt(stringValue.trim()));
                } catch (NumberFormatException ignored) {
                    // Ignore malformed entries and fall back if no valid values remain.
                }
            }
        }
