...
import sora.network.NetworkRequest.RetryMode;
import sora.network.NetworkRequest.RetryPolicy;
...
public final class RetryDecider {
     ...
     */
    private static final Set<Integer> DEFAULT_HARD_NON_RETRY_STATUS = Set.of(
    ...
    private static boolean isStatusRetryable(RetryPolicy policy, int statusCode) {
        ...
        if (hardNonRetryStatus().contains(statusCode)) {
            Logger.log(Logger.TAG.DEBUG,
    ...
    private static long computeBackoffDelay(RetryPolicy policy, int attemptsSoFar) {
        ...
        if (factor <= 0.0) {
            long v = Math.min(base, maxBackoffMs());
            Logger.log(Logger.TAG.DEBUG,
        ...
        if (raw < 0) raw = 0;
        long maxBackoffMs = maxBackoffMs();
    ...
    /** Returns the current cap on computed retry backoff. */
    private static long maxBackoffMs() {
        return Math.max(0L, ConfigManager.getLong("retry_decider.max_backoff_ms", DEFAULT_MAX_BACKOFF_MS));
    }

    /** Returns the configured set of statuses that are never retryable. */
    private static Set<Integer> hardNonRetryStatus() {
        Object raw = ConfigManager.getValue("retry_decider.hard_non_retry_status");
        if (!(raw instanceof JSONArray arr) || arr.length() == 0) {
            return DEFAULT_HARD_NON_RETRY_STATUS;
        }
        ...
        for (int i = 0; i < arr.length(); i++) {
            ...
            if (value instanceof String stringValue) {
                try {
                    ...
                    // Ignore malformed entries and fall back if nothing valid remains.
                }
            }
        }
        return parsed.isEmpty() ? DEFAULT_HARD_NON_RETRY_STATUS : Set.copyOf(parsed);
    }
}
