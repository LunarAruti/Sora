...
final class HttpExecutor {
    ...
    private static volatile Duration runtimeDefaultConnectTimeout = Duration.ofMillis(DEFAULT_CONNECT_TIMEOUT_MS);
     ...
     */
    private static volatile HttpClient DEFAULT_CLIENT = buildClient(runtimeDefaultConnectTimeout);
    ...
    /**
     * Refreshes the cached default connect-timeout value from ConfigManager.
     *
     * The shared default client remains fixed for the current process in v1, so
     * this hook is mainly a centralized read surface for startup and future
     * explicit-apply behavior.
     */
    static synchronized void applyRuntimeConfig() {
        long millis = Math.max(1L, ConfigManager.getLong("http_executor.default_connect_timeout_ms", DEFAULT_CONNECT_TIMEOUT_MS));
        runtimeDefaultConnectTimeout = Duration.ofMillis(millis);
        DEFAULT_CLIENT = buildClient(runtimeDefaultConnectTimeout);
    }
    ...
    private static HttpClient selectClient(Duration connectTimeout) {
        ...
        return CLIENTS_BY_CONNECT_TIMEOUT.computeIfAbsent(
                connectTimeout,
                HttpExecutor::buildClient
        );
    }

    /** Builds an HttpClient with redirects disabled and the supplied connect timeout. */
