...
public final class RateLimiterRegistry {
     ...
     */
    private static final long DEFAULT_MAX_REFILL_ELAPSED_MS = 60_000L; // 60s
                ...
                name -> {
                    Logger.log(Logger.TAG.SYSTEM,
                            "RateLimiterRegistry: auto-create bucket=" + bucketName +
                                    " cap=" + defaultCapacity() + " rps=" + defaultRefillPerSecond());
                    return new Bucket(defaultCapacity(), defaultRefillPerSecond(), nowMillis);
                }
        );
    ...
    public static long estimateDelayMillis(String bucketName, int permits) {
        ...
        Bucket bucket = BUCKETS.computeIfAbsent(
                ...
                name -> {
                    ...
                    Bucket b = new Bucket(defaultCapacity(), defaultRefillPerSecond(), now);
                    b.tokens = 0.0;
                    return b;
                }
        );
    ...
    private static final class Bucket {
        ...
        void refill(long nowMillis) {
            ...
            if (elapsedMillis > maxRefillElapsedMs()) {
                elapsedMillis = maxRefillElapsedMs();
            }
    ...
    /** Returns the default capacity used for auto-created buckets. */
    private static double defaultCapacity() {
        return Math.max(0.000001d, ConfigManager.getDouble("rate_limiter.default_capacity", DEFAULT_CAPACITY));
    }

    /** Returns the default refill rate used for auto-created buckets. */
    private static double defaultRefillPerSecond() {
        return Math.max(0.000001d, ConfigManager.getDouble("rate_limiter.default_refill_per_second", DEFAULT_REFILL_PER_SECOND));
    }

    /** Returns the max wall-clock refill window used to cap jump-based bursts. */
    private static long maxRefillElapsedMs() {
        return Math.max(0L, ConfigManager.getLong("rate_limiter.max_refill_elapsed_ms", DEFAULT_MAX_REFILL_ELAPSED_MS));
    }
}
