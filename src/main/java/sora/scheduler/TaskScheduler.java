...
public final class TaskScheduler implements Runnable {
    private static final long DEFAULT_GRACE_MS = 30L * 60L * 1000L;
    private static final long DEFAULT_SHUTDOWN_AWAIT_MS = 15_000L;
    private static final long DEFAULT_RETRY_BACKOFF_MS = 5_000L;
    private static final long DEFAULT_LOOKAHEAD_MS = 2L * 60L * 60L * 1000L;
    private static final long DEFAULT_REFRESH_INTERVAL_MS = 60L * 60L * 1000L;
    private static final int DEFAULT_MAX_DUE_SOON_TASKS = 1000;

    /** Start-time scheduler grace window for missed one-shots. */
    private static volatile long runtimeGraceMs = DEFAULT_GRACE_MS;

    /** Max time to wait for a clean shutdown. */
    private static volatile long runtimeShutdownAwaitMs = DEFAULT_SHUTDOWN_AWAIT_MS;

    /** Delay before retrying failed tasks. */
    private static volatile long runtimeRetryBackoffMs = DEFAULT_RETRY_BACKOFF_MS;

    /** Registry scan lookahead window for the due-soon cache. */
    private static volatile long runtimeLookaheadMs = DEFAULT_LOOKAHEAD_MS;

    /** Periodic registry refresh interval. */
    private static volatile long runtimeRefreshIntervalMs = DEFAULT_REFRESH_INTERVAL_MS;

    /** Max tasks held in the due-soon cache at once. */
    private static volatile int runtimeMaxDueSoonTasks = DEFAULT_MAX_DUE_SOON_TASKS;
    ...
    public static boolean shutdown() {
        ...
        SchedulerConsoleReader.shutdown();
        if (INSTANCE == null) {
    ...
    /**
     * Loads start-time scheduler tunables from ConfigManager.
     *
     * Scheduler timing constants are intentionally not live-reactive in v1.
     * This method refreshes the cached values used by new scheduler starts while
     * preserving the existing coded defaults for missing or invalid config values.
     */
    public static synchronized void applyRuntimeConfig() {
        runtimeGraceMs = Math.max(0L, ConfigManager.getLong("task_scheduler.grace_ms", DEFAULT_GRACE_MS));
        runtimeShutdownAwaitMs = Math.max(0L, ConfigManager.getLong("task_scheduler.shutdown_await_ms", DEFAULT_SHUTDOWN_AWAIT_MS));
        runtimeRetryBackoffMs = Math.max(0L, ConfigManager.getLong("task_scheduler.retry_backoff_ms", DEFAULT_RETRY_BACKOFF_MS));
        runtimeLookaheadMs = Math.max(0L, ConfigManager.getLong("task_scheduler.lookahead_ms", DEFAULT_LOOKAHEAD_MS));
        runtimeRefreshIntervalMs = Math.max(1L, ConfigManager.getLong("task_scheduler.refresh_interval_ms", DEFAULT_REFRESH_INTERVAL_MS));
        runtimeMaxDueSoonTasks = Math.max(1, ConfigManager.getInt("task_scheduler.max_due_soon_tasks", DEFAULT_MAX_DUE_SOON_TASKS));
    }
    ...
    /**
     * Executes a whitelisted operation immediately using the same scheduler-owned
     * command executor used for persisted tasks.
     *
     * This does not create or persist a ScheduledTask. It is intended for
     * alternate trigger surfaces, such as console input, that should share the
     * exact same execution path as scheduled operations.
     *
     * @param opKey whitelisted operation key
     * @param opArgs comma-separated argument string expected by the scheduler executor
     * @return short result message from the executor
     * @throws TaskException when the scheduler is not running or execution fails
     */
    public static String executeImmediate(String opKey, String opArgs) throws TaskException {
        if (INSTANCE == null) {
            throw new TaskException("TaskScheduler.executeImmediate: scheduler not started.");
        }
        return INSTANCE.executeImmediateInternal(opKey, opArgs);
    }
    ...
    public boolean start() throws TaskException {
        ...
        Thread t = new Thread(this, "Task Scheduler");
        t.setDaemon(true);
        workerThread.set(t);
    ...
    /**
     * Executes a single operation immediately through the scheduler's shared
     * command executor without creating a scheduled task.
     *
     * @param opKey whitelisted operation key
     * @param opArgs comma-separated argument string
     * @return short result message from the executor
     * @throws TaskException when the scheduler is not running or the executor fails
     */
    private String executeImmediateInternal(String opKey, String opArgs) throws TaskException {
        if (!running.get()) {
            throw new TaskException("TaskScheduler.executeImmediate: scheduler is not running.");
        }
        if (opKey == null || opKey.isBlank()) {
            throw new IllegalArgumentException("TaskScheduler.executeImmediate: opKey is null/blank.");
        }

        String normalizedKey = opKey.trim().toUpperCase(Locale.ROOT);
        String normalizedArgs = (opArgs == null) ? "" : opArgs;

        Logger.log(Logger.TAG.INFO,
                "TaskScheduler: immediate execute opKey=" + normalizedKey);
        try {
            String result = executor.execute(normalizedKey, normalizedArgs);
            Logger.log(Logger.TAG.INFO,
                    "TaskScheduler: immediate execute success opKey=" + normalizedKey +
                            " result=" + result);
            return result;
        } catch (Exception e) {
            Logger.log(Logger.TAG.ERROR,
                    "TaskScheduler: immediate execute failed opKey=" + normalizedKey +
                            " err=" + safeMsg(e.getMessage()));
            throw new TaskException(
                    "TaskScheduler immediate execution failed for opKey=" +
                            normalizedKey + ": " + safeMsg(e.getMessage()),
                    e
            );
        }
    }
    ...
    private void refreshFromRegistry(long now, String reason, boolean throwOnFailure) throws TaskException {
        ...
        long localNextRefresh = now + runtimeRefreshIntervalMs;
        if (dueSoon.size() > runtimeMaxDueSoonTasks) {
            long firstDroppedDue = dueSoon.get(runtimeMaxDueSoonTasks).dueAt;
            dueSoon = new ArrayList<>(dueSoon.subList(0, runtimeMaxDueSoonTasks));
            long refreshEarlyAt = Math.max(now + 5_000L, firstDroppedDue - 30_000L);
            localNextRefresh = Math.min(localNextRefresh, refreshEarlyAt);
            Logger.log(Logger.TAG.WARN, "[13017] TaskScheduler: due-soon cache capped at " + runtimeMaxDueSoonTasks +
    ...
    private boolean isDueSoon(long dueAt, long now) {
        return dueAt <= now + runtimeLookaheadMs;
    }
    ...
    private boolean isMissed(long dueAt, long now) {
        return now > (dueAt + runtimeGraceMs);
    }
