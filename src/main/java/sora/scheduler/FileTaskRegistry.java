...
public final class FileTaskRegistry implements TaskRegistry {
    ...
    private static final String REGISTRY_PATH = Path.of(BootstrapConfig.REGISTRYPATH, "tasks.json").toString();
    private static final String DEFAULT_TASKS_KEY = "tasks";
    private static volatile String runtimeTasksKey = DEFAULT_TASKS_KEY;
    ...
    /**
     * Loads start-time registry tunables from ConfigManager.
     *
     * The registry file path is bootstrap/static, but the JSON root key is stored
     * in runtime config for centralization and fallback validation. Because this
     * value is flagged as non-live-updating, the registry caches it when new
     * registry instances are created.
     */
    public static synchronized void applyRuntimeConfig() {
        String configured = ConfigManager.getString("registry.tasks_key", DEFAULT_TASKS_KEY);
        if (configured == null) {
            runtimeTasksKey = DEFAULT_TASKS_KEY;
            return;
        }

        String normalized = configured.trim();
        runtimeTasksKey = normalized.isEmpty() ? DEFAULT_TASKS_KEY : normalized;
    }
    ...
    public List<ScheduledTask> loadAllActive() throws TaskException {
        ...
        JSONObject tasks = root.optJSONObject(runtimeTasksKey);
        if (tasks == null) {
    ...
    public ScheduledTask get(String taskId) throws TaskException {
        ...
        Object raw;
        try {
            raw = DatabaseManager.readJSONPath(REGISTRY_PATH, runtimeTasksKey + "." + taskId);
        } catch (DatabaseException e) {
            if (ensureRegistryIfMissing()) {
                try {
                    raw = DatabaseManager.readJSONPath(REGISTRY_PATH, runtimeTasksKey + "." + taskId);
    ...
    public void delete(String taskId) throws TaskException {
        ...
        try {
            DatabaseManager.removeJSONPath(REGISTRY_PATH, runtimeTasksKey + "." + taskId);
        } catch (DatabaseException e) {
            if (ensureRegistryIfMissing()) {
                try {
                    DatabaseManager.removeJSONPath(REGISTRY_PATH, runtimeTasksKey + "." + taskId);
    ...
    public void markTerminal(String taskId, ScheduledTask.Status status, long now, String result) throws TaskException {
        ...
        try {
            Object raw = DatabaseManager.readJSONPath(REGISTRY_PATH, runtimeTasksKey + "." + taskId);
            if (!(raw instanceof JSONObject)) {
                throw new TaskException("TaskRegistry markTerminal: missing task " + taskId);
            }
            DatabaseManager.writeJSONPath(REGISTRY_PATH, runtimeTasksKey + "." + taskId + ".status", status.name(), true);
            DatabaseManager.writeJSONPath(REGISTRY_PATH, runtimeTasksKey + "." + taskId + ".updatedAt", now, true);
            DatabaseManager.writeJSONPath(REGISTRY_PATH, runtimeTasksKey + "." + taskId + ".lastResult", result, true);
        } catch (DatabaseException e) {
            if (ensureRegistryIfMissing()) {
                try {
                    Object raw = DatabaseManager.readJSONPath(REGISTRY_PATH, runtimeTasksKey + "." + taskId);
    ...
    private boolean ensureRegistryIfMissing() throws TaskException {
        ...
        JSONObject root = new JSONObject().put(runtimeTasksKey, new JSONObject());
        try {
    ...
    private static void markBadTask(String taskId, JSONObject obj, String reason) {
        ...
        try {
            DatabaseManager.writeJSONPath(REGISTRY_PATH, runtimeTasksKey + "." + taskId, err, true);
        } catch (DatabaseException e) {
